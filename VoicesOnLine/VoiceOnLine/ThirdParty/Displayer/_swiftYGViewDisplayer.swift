...
public class _swiftYGViewDisplayOptions: NSObject {
    ...
    @objc public var screenBackgroundColor: UIColor = UIColor(white: 100.0/255.0, alpha: 0.2)
    /// 内容区背景色，默认为white
...
public class _swiftYGViewDisplayer: NSObject {
    @objc static public func useWindowScene(_ value: Bool) {
    ...
    /// 底部弹出到中间
    @objc static public func popupCenter(_ view: UIView, setupBlock :((_swiftYGViewDisplayOptions)->())?) {
        ...
        self.ekDisplay(view, attributes)
    }
    
    /// 底部弹出固定在底部，默认宽度充满屏幕
    @objc static public func popupBottom(_ view: UIView, setupBlock :((_swiftYGViewDisplayOptions)->())?) {
        let options = _swiftYGViewDisplayOptions()
        options.priority = .max
    ...
    /// 淡出在中间
    @objc static public func fadeCenter(_ view: UIView, setupBlock :((_swiftYGViewDisplayOptions)->())?) {
        let options = _swiftYGViewDisplayOptions()
        options.positionConstraints = .float
        options.safeArea = .overridden
        options.priority = .max
    ...
    @objc public static func display(_ view: UIView, setupBlock: ((_swiftYGViewDisplayOptions)->())?) {
        let options = _swiftYGViewDisplayOptions()
    ...
    private struct AssociatedKeys { static var displayIdentity = "displayIdentity"}
    ...
    /// dismiss某一个view
    @objc public static func dismiss(_ view: UIView, completionHandler: (()->())?) {
    ...
    /// dismiss当前
    @objc public static func dismissDisplayed(_ completionHandler: (()->())?) {
        SwiftEntryKit.dismiss(.displayed, with: completionHandler)
    }
    
    @objc public static func dismiss(identity: String, completionHandler: (()->())?) {
        SwiftEntryKit.dismiss(.specific(entryName: identity), with: completionHandler)
    }
    
    /// 移出所有，当前显示的和队列中的
    @objc public static func dismissAll(_ completionHandler: (()->())?) {
