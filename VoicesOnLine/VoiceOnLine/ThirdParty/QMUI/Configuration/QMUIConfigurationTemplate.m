...
- (void)applyConfigurationTemplate {
    ...
    if (@available(iOS 15.0, *)) {
        QMUICMI.navBarUsesStandardAppearanceOnly = true;                         // NavBarUsesStandardAppearanceOnly : 对于 iOS 15 的系统，UINavigationBar 的样式分为滚动前和滚动后，虽然系统的注释里说了如果没设置 scrollEdgeAppearance 则会用 standardAppearance 代替，但实际运行效果是 scrollEdgeAppearance 默认并不会保持与 standardAppearance 一致，所以这里提供一个开关，允许你在打开开关时让 QMUI 帮你同步 standardAppearance 的值，以使 App 保持与 iOS 14 相同的效果。如需打开该开关，请保证在其他 NavBar 开关之前设置。
    }
    ...
    QMUICMI.navBarButtonFontBold = UIFontMake(18);                                         // NavBarButtonFontBold : QMUINavigationButtonTypeBold 的字体
    QMUICMI.navBarBackgroundImage = nil;//UIImageMake(@"navigationbar_background");                                        // NavBarBackgroundImage : UINavigationBar 的背景图
    if (@available(iOS 15.0, *)) {
    ...
    QMUICMI.navBarShadowImage = nil;//[UIImage new];                                            // NavBarShadowImage : UINavigationBar.shadowImage，也即导航栏底部那条分隔线，配合 NavBarShadowImageColor 使用。
    ...
    QMUICMI.navBarTintColor = UIColorMakeWithHex(@"#1F1F1F");                                              // NavBarTintColor : QMUINavigationController.navigationBar 的 tintColor，也即导航栏上面的按钮颜色，由于 tintColor 不支持 appearance，所以这里只支持 QMUINavigationController
    QMUICMI.navBarTitleColor = UIColorMakeWithHex(@"#0E0E0E");                                             // NavBarTitleColor : UINavigationBar 的标题颜色，以及 QMUINavigationTitleView 的默认文字颜色
    ...
    QMUICMI.navBarBackIndicatorImage = [UIImage qmui_imageWithShape:QMUIImageShapeNavBack size:CGSizeMake(16, 16) tintColor:NavBarTintColor];                                     // NavBarBackIndicatorImage : 导航栏的返回按钮的图片，图片尺寸建议为(13, 21)，否则最终的图片位置无法与系统原生的位置保持一致
    ...
    QMUICMI.tabBarBackgroundImage = [[UIImage qmui_imageWithColor:UIColorMakeWithHex(@"#FFFFFF")] resizableImageWithCapInsets:UIEdgeInsetsMake(1, 1, 1, 1)];                                        // TabBarBackgroundImage : UITabBar 的背景图
    if (@available(iOS 15.0, *)) {
    ...
    QMUICMI.tabBarShadowImageColor = [UIColorMakeWithHex(@"#000000") colorWithAlphaComponent:0.03];                                       // TabBarShadowImageColor : UITabBar 的 shadowImage 的颜色，会自动创建一张 1px 高的图片
    QMUICMI.tabBarStyle = UIBarStyleDefault;                                    // TabBarStyle : UITabBar 的 barStyle
    QMUICMI.tabBarItemTitleFont = UIFontMake(18);                                          // TabBarItemTitleFont : UITabBarItem 的标题字体
    QMUICMI.tabBarItemTitleColor = UIColorMakeWithHex(@"#BFBFBF");                                         // TabBarItemTitleColor : 未选中的 UITabBarItem 的标题颜色
    QMUICMI.tabBarItemTitleColorSelected = UIColorMakeWithHex(@"#00B58A");                                 // TabBarItemTitleColorSelected : 选中的 UITabBarItem 的标题颜色
    QMUICMI.tabBarItemImageColor = UIColorMakeWithHex(@"#BFBFBF");                                       // TabBarItemImageColor : UITabBarItem 未选中时的图片颜色
    QMUICMI.tabBarItemImageColorSelected = UIColorMakeWithHex(@"#00B58A");                                 // TabBarItemImageColorSelected : UITabBarItem 选中时的图片颜色
    ...
    #pragma mark - QMUILog
    QMUICMI.shouldPrintDefaultLog = NO;                                        // ShouldPrintDefaultLog : 是否允许输出 QMUILogLevelDefault 级别的 log
    QMUICMI.shouldPrintInfoLog = NO;                                           // ShouldPrintInfoLog : 是否允许输出 QMUILogLevelInfo 级别的 log
    QMUICMI.shouldPrintWarnLog = NO;                                           // ShouldPrintInfoLog : 是否允许输出 QMUILogLevelWarn 级别的 log
    ...
    QMUICMI.automaticCustomNavigationBarTransitionStyle = true;                   // AutomaticCustomNavigationBarTransitionStyle : 界面 push/pop 时是否要自动根据两个界面的 barTintColor/backgroundImage/shadowImage 的样式差异来决定是否使用自定义的导航栏效果
...
- (UIColor *)themeTintColor {
    return UIColorMakeWithHex(@"#FF82A2");
}

- (UIColor *)themeBackgroundColor {
    return UIColorMakeWithHex(@"#F6F7FB");
}
...
- (UIColor *)themeDescriptionTextColor {
    return VL_CONTENT_COLOR;
}

- (UIColor *)themePlaceholderColor {
    return VL_PLACEHOLD_COLOR;
}
